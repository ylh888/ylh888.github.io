...
cam2sound_2.js : 2022-03-03 Thu :  2020-10-29 Thu : see below

*/
/* set1 = I hear hue vignettes
...
if (SET === "I hear hue") {
  nVignettes = 4
  set_options = 2 // 1:"I hear hue" 2:"I hear light"
}
...
if (SET === "drive MIDI") {
  nVignettes = 1 // 0: horizontal channels 1: vertical channels 
  set_options = 1 // 
  uw = uh = 10
  mw = 220
  mh = 140
...
function setup() {
  ...
  */
  ...
  samplingRateSlider = createSlider(15, 3000, 15, 15);
...
function draw() {
  if (myCapture !== null && myCapture !== undefined) { // safety first
    ...
    */
    if (SET == "I hear hue") {
      image(myVida.currentImage, 0, camerawidth);
      image(myVida.backgroundImage, cameraheight, camerawidth);
      image(myVida.differenceImage, 0, 0);
      image(myVida.thresholdImage, cameraheight, 0);
    } else if (SET == "drive MIDI") {
      // image(myVida.currentImage, 0, 240);
      // image(myVida.backgroundImage, 320, 240);
      // image(myVida.differenceImage, 320, 0);
    ...
    */
    ...
    var offset_x = camerawidth;
    var offset_y = cameraheight;
...
let uchans = [],
  mchans = [] // min and max; each nchans wide 

function ylhDriveMIDI() { //===============================================
  ...
  if (millis() > nextSample) {
    ...
    if (vignette === 0) { // horizontal 
      ...
      for (let j = 0; j < nChans; j++) {
        ...
        for (let i = 0; i < vPix; i++) {
          x = uw + (localw / (vPix + 1)) * (i + 1)
          thispixel = brightness(get(x, y))
          if (thispixel > brightnessThreshold) {
            uchans[j] = i
            break
          }
        }
        for (let i = vPix - 1; i >= 0; i--) {
          x = uw + (localw / (vPix + 1)) * (i + 1)
          thispixel = brightness(get(x, y))
          if (thispixel > brightnessThreshold) {
            mchans[j] = i
        ...
        if (vocoder) { //=========================
          ...
          midiNum = map(uchans[j], 0, vPix, 1, 126)
          myOutput.channels[j+1].playNote(midiNum, {
            duration: 500
          })
          midiNum = map(mchans[j], 0, vPix, 1, 126)
          myOutput.channels[j+1+nChans].playNote(midiNum, {
            duration: 500
          })
        }
    ...
    // console.log("uchans" + uchans)
    // console.log("mchans" + mchans)
  }
  ...
  let idx = sample % (camerawidth * cameraheight)
  ...
  for (let i = 0; i < nChans; i++) {
    stroke(20, 20, 200)
    fill(250, 250, 0) // yellow - right
    circle(uw + uchans[i] * localw / (vPix + 1), uh + (i + 1) * localh / (nChans + 1), 5)
    fill(250, 0, 250) // purple  - left
    circle(uw + mchans[i] * localw / (vPix + 1), uh + (i + 1) * localh / (nChans + 1), 5)
  }
  ...
  if (set_options == 1) {
    noStroke();
    fill(255, 255, 255);
    text('horizontal channels', 10, 15);
...
function ylhIHEARHUE() {
  ...
  if (millis() > nextSample) {
    ...
    if (vignette === 0) { // horizontal 
      ...
      uw = 0, uh = 0, mw = camerawidth, mh = cameraheight
      x = camerawidth / 2
      y = cameraheight / 2
    }
    resample = true;
...
function windowResized() {
  resizeCanvas(camerawidth * 2, cameraheight * 2 + 100);
  lheight = height - 60
  uw = uh = 0
  mw = camerawidth
  mh = cameraheight
